...
public class QBAnimationNew : MonoBehaviour {
	...
	void Start () {
		...
		Lever = GameObject.FindGameObjectWithTag ("Lever").GetComponent<AudioSource> ();
	...
	void Update () {
		...
		bossBat = GameObject.Find ("manager").GetComponent<Xob2> ().qbCount;
		...
		foreach (Touch t in Input.touches) {
			if (t.phase == TouchPhase.Began) {
				...
				if (Input.touchCount == 1 && distance > 100f) { //if ONE finger is swiped at a distance 100 or greater 
					...
					else if ((!swipedSideways && deltaY > 0) && ((initialTouch.position.x) > mid)) {//swiped down on the right
					...
					else if ((!swipedSideways && deltaY > 0) && ((initialTouch.position.x) < mid)) {//swiped down on the left
						//this.transform.Rotate (new Vector3(0, 100f, 0));
						Lever.Play();
					...
					//swiped up
					else if ((!swipedSideways && deltaY <= 0) && ((initialTouch.position.x) > mid)) {//swiped up on the right
						Lever.Play();
					...
					else if ((!swipedSideways && deltaY <= 0) && ((initialTouch.position.x) < mid)) {//swiped up on the left
						Lever.Play();
