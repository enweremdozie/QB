...
public class DiscoEnd : MonoBehaviour {
	...
	IEnumerator waitSomeTime() {//coroutine if still needed at some point
		...
		if (endCount != 1) {
			...
			//GameObject.Find ("manager").GetComponent<EndGame> ().End();
			GameObject.Find ("manager").GetComponent<Trail> ().begin ();
			//Debug.Log ("This is count " + count);
			//GameObject.Find ("manager").GetComponent<Trail> ().rail(38,39);
		}
	}//end of waitSomeTime()
